  ...
  try {
    ...
    for (const [streamId, stepOrder] of boundedHits) {
      ...
      jobs.push({
        streamId,
        channelId: id,
        // Routed through Dispatcharr when the setting says so, exactly as the
        // worker routes its own probes -- an on-demand check that measured a
        // different path from the pass would rank against numbers the pass
        // could not reproduce.
