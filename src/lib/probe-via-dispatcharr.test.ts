...
describe('activeSessions with Podium as a client', () => {
  ...
  it('counts its own probes as viewers when told nothing', () => {
    ...
    return expect(client().activeSessions()).resolves.toEqual([{ channelId: 8, profileId: 9 }]);
  });
  ...
  it('keeps a session a real viewer is sharing with a probe', async () => {
    ...
    expect(sessions).toEqual([{ channelId: 8, profileId: 9 }]);
  });
